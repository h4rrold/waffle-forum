...
class Topic extends Model
{
    ...
    public function get5Topics($first, $second)
    {
        ...
        for ($i = 0; $i < count($topics); $i++){
            ...
            $topics[$i]['datetime'] = $date->format('j ').$this->monthes[date('n')].$date->format(' Y');
        }
        return $topics;
    }
    ...
    public function getTopic($id_topic)
    {
        ...
        $topic['datetime'] = $date->format('j ').$this->monthes[date('n')].$date->format(' Y');
        return $topic;
    }
}