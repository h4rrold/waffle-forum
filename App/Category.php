...
class Category extends Model
{
    ...
    public function getFetchCategories()
    {
        ...
        for ($i = 0; $i < count($categories); $i++){
            ...
            $categories[$i]['amount_of_posts'] = MyPDO::first("SELECT COUNT(posts.id) as count FROM posts, topics, directories WHERE topics.directory_id = directories.id AND posts.topic_id = topics.id AND directories.id = ?", [$categories[$i]['id']])['count'] - 1;
